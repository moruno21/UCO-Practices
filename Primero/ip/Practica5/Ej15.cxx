...
bool find_in_sorted_vector(int val, int v[], int n){
	for (int i=0; i<n; i++){
		if (v[i]==val){//Solo entramos si val esta en el vector
			return true;//Devolvemos true, indicando que val está en el vector, y finalizamos
		}
	}
return false;//Después de comprobar todos los elementos, val no estaría en el vector, por lo tanto se devolvería false y se finalizaría
}
int test();
int main(){ 
	 test();
