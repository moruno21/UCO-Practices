...
void ordena_vector(int v[], int n){
	for (int i=0; i<n-1; i++){
		for (int j=i+1; j<n; j++){
			if (v[i]>v[j]){//Entramos solo si el valor i es mayor que el siguiente valor del vector (j)
				int a=v[i];//Guardamos en a el valor del mayor de los dos numeros comparados
				v[i]=v[j];//Guardamos en la posición primera  el menor valor
				v[j]=a;//Guardamos el mayor valor (previamente guardado en a) en la posicion j
			}
		}
	}
}
