...
int find_indices(int val, int v[], int n, int indices[]){
	...
	int j=0;
	for (int i=0; i<n; i++){
		if (v[i]==val){//Entramos si val esta en el vector v[]
			indices[j]=i;//Guardamos como primer elemento del vector indices[] la posición en la que se encuentra el primer val en el vector v[]
			j++;
			cont++;//Contamos cuantas veces aparece val en el vector v[]
		}
	}
return  cont;//Devuelve el total de veces que aperece val en el vector v[]
}
